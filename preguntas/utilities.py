...
def make_name():
    raw = make_text(random.randint(10,25))
    exclude = set(string.punctuation + ' ')
    stripped = ''.join(ch for ch in raw if ch not in exclude)
...
def make_username():
    raw = make_text(random.randint(5,10))
    exclude = set(string.punctuation + ' ')
    stripped = ''.join(ch for ch in raw if ch not in exclude)
...
def make_random_questions():
    for each_usuario in Usuario.objects.all():
        for _ in range(random.randint(0,10)):
            Question(title=make_text(random.randint(25,255)), body=make_text(random.randint(50,5000)),owner=each_usuario).save()

def make_random_answers():
    number_of_usuarios = Usuario.objects.count()
    for each_question in Question.objects.all():
        for _ in range(random.randint(0,50)):
            Answer(body=make_text(random.randint(25,7500)), question=each_question, owner=Usuario.objects.all()[random.randint(0,number_of_usuarios-1)]).save()
